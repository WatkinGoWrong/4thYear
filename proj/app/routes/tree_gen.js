...
function tree() {
  ...
  tree.removeLeaf = function(nodeClicked) {
    ...
    function findParent(node) { //finds the parent object of the leaf clicked and saves it to parent variable
      ...
      if (node.id == nodeClicked.id) {
        visited.pop();
        parent = visited[visited.length - 1];
        removeLeaf(tree.nodes[0]);
      } else {
  ...
  tree.changeText = function(node) {
    ...
    var textBox = d3.select("#nodes").append("foreignObject").attr("x", node.x + 50).attr("y", node.y)
      ...
      .append("input").attr("value", function() {
        this.focus();
        return node.text
      })
      .on("blur", function() {
    ...
    nodes.enter().append('text').attr('id', function(node) { /*console.log('id = ' + node.id);*/
      ...
      .on('click', function(node) {
        if (d3.event.shiftKey) {
          return tree.changeText(node);
        } else if (d3.event.ctrlKey) {
          return tree.removeLeaf(node);
        } else {
          return tree.addLeaf(node.id);
        }
      })
      /*.transition().duration(500)*/
      .attr('x', function(node) {
